...
#'
#' @param a \code{data.frame} \code{\link[data.table]{data.table}}, or \code{\link[tibble]{tibble}}
#'      recording tweets.
#'      For required column (naming and typing conventions) refer to \code{?\link{required.tweets.df.cols}}.
#'      For an example see \code{?\link{tweets.df.prototype}}.
#'
...
create_tweet_features <- function(
  ...
  # helper
  col_msg <- function(col.idxs) {
    sprintf(
      "\n  - %s (%s)"
      , sQuote(.req.columns.mapping$colname[col.idxs])
      , vapply(.req.columns.mapping$accepted_classes[col.idxs], paste, NA_character_, collapse = "/")
    )
  }
  ...
  if (any(!col_class_check)) {
    idxs_ <- which(.req.columns.mapping$colname %in% names(col_class_check)[!col_class_check])
    stop("The following columns of `x` have wrong classes (correct classes in parantheses): ", col_msg(idxs_), call. = FALSE)
  }
